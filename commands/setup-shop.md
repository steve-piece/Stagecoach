...
- **Flow A — First-time install (system-wide).** Triggers when `~/.bytheslice/defaults.json` doesn't exist. Asks the per-section questions once and saves them as your machine-wide defaults so future projects can opt in via a single "use defaults?" question.
- **Flow B — New project (Bootstrap + Config).** Triggers when the working directory has no `package.json`. Scaffolds a Next.js single-app or Turborepo monorepo via the official scaffolders, then drops in a per-project `bytheslice.config.json` and a gitignored `ROADMAP.local.md`.
