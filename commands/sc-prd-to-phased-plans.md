...
Load and follow the [`prd-to-phased-plans`](../skills/prd-to-phased-plans/SKILL.md) skill.
...
- A finalized PRD exists (output of `/sc-prd-generator` or equivalent). Raw briefs, specs, and questionnaires feed `/sc-prd-generator` first.

## When to use this command

Use `/sc-prd-to-phased-plans` once you have a completed PRD and are ready to produce the full implementation plan. The output is the input for `/sc-the-orchestrator` or individual stage commands (`/sc-feature-delivery`, `/sc-ci-cd-scaffold`).
