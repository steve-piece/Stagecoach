...
This file was generated by the `/sp-design-system-gate` skill.
...
To regenerate or update this file, run `/sp-design-system-gate` with the updated bundle or brief. Do not edit token values directly without also updating `globals.css` and `tailwind.config.ts`.
```
