...
This file was generated by the `/sc-design-system-gate` skill.
...
To regenerate or update this file, run `/sc-design-system-gate` with the updated bundle or brief. Do not edit token values directly without also updating `globals.css` and `tailwind.config.ts`.
```
