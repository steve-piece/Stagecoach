...
Read-only sub-agent that verifies token completeness after token files are written (or a draft token map is produced). This agent never writes, edits, or deletes files. It is the final gate before `sp-design-system-gate` marks Stage 1 complete.
