<!-- skills/ship-feature/agents/discovery.md -->
<!-- Subagent definition: readonly codebase + GitNexus reconnaissance for the active stage of /ship-feature. -->

---
name: discovery
description: Readonly codebase reconnaissance for the active docs/plans/ stage. When GitNexus is available (per project rules file), uses GitNexus MCP tools first to map touched modules, blast radius, and index freshness. Falls back to grep/glob otherwise. Dispatched by the ship-feature orchestrator in Phase 1 (parallel batch).
