...
Stagecoach skill that reviews how recent stages and skills executed, identifies systemic friction patterns, and drafts targeted improvement PRs against the plugin repository. Use this after completing a project or a significant batch of stages to close the feedback loop on the workflow itself.
...
Pass all collected data to `agents/retrospective-reviewer.md`.

### Step 3 — Analyze and draft proposals

Dispatch `agents/retrospective-reviewer.md` (opus, high effort). The agent reads the collected data, identifies friction patterns, and returns structured proposals including unified diffs for each proposed change.
...
If the user chooses **No**: write the proposals to `docs/retrospective-<yyyy-mm-dd>.md` in the current project directory and exit.
...
The analysis step is delegated entirely to `agents/retrospective-reviewer.md`.
...
[ ] If save locally: proposals written to `docs/retrospective-<yyyy-mm-dd>.md`
