...
Read-only sub-agent that performs cross-stage friction analysis on behalf of the `phased-dev-retrospective` skill. This agent receives execution data from the skill, detects systemic patterns, and returns structured proposals including unified diffs for each proposed plugin change.
...
The `phased-dev-retrospective` skill passes the following at dispatch time:
