...
Every stage plan file produced by `prd-to-phased-plans` (and consumed by `the-orchestrator`) must begin with this YAML frontmatter block. All fields are mandatory. No omissions, no extras.
...
- `skills/prd-to-phased-plans/agents/` — all stage writer agents
- `skills/the-orchestrator/SKILL.md` — for stage routing by `type`
- `skills/sp-feature-delivery/SKILL.md` — receives stage frontmatter as execution context
