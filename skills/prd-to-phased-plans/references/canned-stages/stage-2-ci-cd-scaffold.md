...
<!-- Stage 2: bootstrap CI/CD + E2E baseline via sp-ci-cd-scaffold skill -->

# Stage 2 — CI/CD Scaffold

**Goal:** Bootstrap CI/CD + E2E baseline before any feature work begins.

**Architecture:** This stage delegates entirely to the `sp-ci-cd-scaffold` skill. It establishes Playwright suites (`@feature`, `@regression-core`), GitHub Actions workflows (`ci.yml`, `e2e.yml`), Husky `pre-push`, PR template, and branch-protection setup. Every later stage depends on this baseline.
...
### Task 1: Run sp-ci-cd-scaffold skill end-to-end

This stage's task list is not duplicated here. The single task is:

[ ] Run the `sp-ci-cd-scaffold` skill in full.

The skill's `SKILL.md` (Phases 0–6) is the source of truth for the work. Its completion checklist (embedded in `sp-ci-cd-scaffold/SKILL.md`) is the source of truth for "done".
...
**Commit:** delegated to sp-ci-cd-scaffold skill (conventional commits per task within that skill)

---

**Exit criteria:**
- Every completion criterion in `sp-ci-cd-scaffold` skill is satisfied
