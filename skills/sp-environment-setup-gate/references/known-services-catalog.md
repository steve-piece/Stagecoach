<!-- skills/sp-environment-setup-gate/references/known-services-catalog.md -->
<!-- Extensible catalog mapping env var prefix patterns to external services. Used by sp-environment-setup-gate Phase 1 to detect which services a project requires. To add a new service: append a new entry following the ENTRY FORMAT below. -->

# Known Services Catalog

This catalog maps environment variable prefix patterns to external service identities. The `sp-environment-setup-gate` skill uses this during Phase 1 (service detection) to group keys by service and generate targeted provisioning checklists.
