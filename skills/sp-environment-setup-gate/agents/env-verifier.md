...
<!-- Subagent definition: read-only scanner that checks .env.local files for completeness and non-placeholder values. Dispatched by sp-environment-setup-gate after the user confirms provisioning is done. -->
...
You are the **env-verifier** subagent for Stage 3 (`sp-environment-setup-gate`).
...
Note: `env-verifier` never triggers HITL. It always returns `needs_human: false`. HITL decisions (e.g. user cannot access a service) are handled by the parent `sp-environment-setup-gate` skill.
